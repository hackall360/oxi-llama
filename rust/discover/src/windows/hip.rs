...
impl HipLibrary {
    fn load() -> Result<Self, String> {
        let candidates = ["amdhip64.dll", "hip_hcc.dll"];
    ...
    unsafe fn device_bus_id(&self, device: hipDevice_t) -> Result<String, String> {
        match self.hip_device_get_pci_bus_id {
            Some(func) => {
            ...
            None => Err("hipDeviceGetPCIBusId symbol missing".to_string()),
        }
    }
    ...
    unsafe fn device_mem_info(&self, device: hipDevice_t) -> Result<MemInfo, String> {
        ...
        let mut free_bytes = total_bytes;
        if hip_check((self.hip_set_device)(device), "hipSetDevice").is_ok() {
            let mut runtime_total: usize = 0;
            if hip_check(
                (self.hip_mem_get_info)(
                    &mut free_bytes as *mut usize,
                    &mut runtime_total as *mut usize,
                ),
                "hipMemGetInfo",
            )
            .is_err()
