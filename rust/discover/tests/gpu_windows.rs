...
fn test_process_system_logical_processor_information_list() {
    ...
    for (name, b64, expected) in test_cases {
        ...
        for (i, pkg) in expected.iter().enumerate() {
            ...
            assert_eq!(resp[i].efficiency_core_count, pkg.efficiency, "[{name}] efficiency");
