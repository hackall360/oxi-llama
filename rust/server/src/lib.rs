...
use api::{
    ChatResponse, CreateRequest, DeleteRequest, EmbedResponse, GenerateRequest, GenerateResponse,
    ListModelResponse, ListResponse, Message as ApiMessage, ShowResponse,
};
use axum::{
...
pub async fn openai_completion_handler(
...
pub async fn openai_model_handler(
    State(state): State<Arc<AppState>>,
    Path(model): Path<String>,
) -> Result<Json<openai::Model>, StatusCode> {
    let models = state.models.lock().unwrap();
    if let Some(m) = models.get(&model) {
        let resp = ShowResponse {
            modified_at: Some(m.modified_at.clone()),
            ..Default::default()
        };
...
fn verify_signature(header: &str, message: &[u8]) -> bool {
    ...
    let verifying = match VerifyingKey::from_bytes(&key_array) {
        Ok(v) => v,
        Err(_) => return false,
    };
