...
async fn client_from_environment() {
    struct Case { value: &'static str, expect: &'static str }
    let cases = vec![
        Case{ value:"", expect:"http://127.0.0.1:11434"},
        Case{ value:"1.2.3.4", expect:"http://1.2.3.4:11434"},
        Case{ value:":1234", expect:"http://127.0.0.1:1234"},
        Case{ value:"1.2.3.4:1234", expect:"http://1.2.3.4:1234"},
        Case{ value:"http://1.2.3.4", expect:"http://1.2.3.4:80"},
        Case{ value:"https://1.2.3.4", expect:"https://1.2.3.4:443"},
        Case{ value:"https://1.2.3.4:1234", expect:"https://1.2.3.4:1234"},
    ];
    for c in cases {
...
fn start_server(body: String, status: u16, ct: &str) -> (Url, thread::JoinHandle<()>) {
    ...
    let handle = thread::spawn(move || {
        if let Ok(request) = server.recv() {
            let resp = Response::new(status.into(), vec![ct_header], body.as_bytes(), Some(body.len()), None);
...
async fn client_stream() {
    ...
    for (resps, want_err) in cases {
        let body = resps.iter().map(|r| match r {
        ...
        let res = client.stream(Method::POST, "/v1/chat", None::<&()>, |b| {
...
async fn client_do() {
    ...
    for resp in cases {
        let (body, status) = match resp { Resp::Json(b,s) => (b.to_string(), s) };
        let (url, handle) = start_server(body, status, "application/json");
        let client = Client::new(url, reqwest::Client::new());
        #[derive(Default, Deserialize)]
        struct R { id: Option<String>, success: Option<bool> }
        let result: Result<R, Error> = client.do_request(Method::POST, "/v1/messages", None::<&()>) .await;
        if status != 200 {
